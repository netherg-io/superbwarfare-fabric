...
Stacked on superbwarfare-fabric PR #1 (`fix/drone-control-authority`, baseline a31374830d50ad6d4345c765a58ef0545c90671c). This PR reuses its `DroneControlAccess` resolver, including its float-overflow input correction. The earlier competing Kotlin policy and duplicate packet rewrites were removed from this branch's final tree, along with temporary review workflows. Do not merge two authority implementations.
...
`bash dev/test-drone-control.sh` runs the production authorization-rule checks from prerequisite PR #1 (182 in the pinned baseline). `python3 dev/check-drone-input-reset.py` is a source-level guard for pre-tick placement and reuse of that resolver. These checks do not prove actual Fabric adapter loading, Minecraft physics or multiplayer behavior. Check current-head full Build separately; no passing full build is inferred from isolated tests.
