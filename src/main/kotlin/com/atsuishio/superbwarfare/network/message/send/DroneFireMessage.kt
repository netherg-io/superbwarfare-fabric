...
data class DroneFireMessage(val pos: SerializedVector3f) : ServerPacketPayload() {
    override fun PayloadContext.handler() {
        val player = sender()
        val drone = DroneControlAccess.activeDrone(player) ?: return
        if (player.offhandItem.`is`(ModItems.FIRING_PARAMETERS, ModItems.ARTILLERY_INDICATOR)) {
            // Do not convert NaN/infinity to integer target coordinates.
            if (!DroneControlRules.finiteTarget(pos.x, pos.y, pos.z)) return
            ...
            // Keep the virtual setter: ScoutDroneEntity intentionally ignores fire.
            drone.fire = true
        }
    }
}
